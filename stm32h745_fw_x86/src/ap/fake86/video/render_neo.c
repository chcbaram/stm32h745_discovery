...
static uint32_t _temp[320 * 240] __attribute__((section(".sdram_buf")));
uint8_t _frame_buf[480 * 272 * 3] __attribute__((section(".sdram_buf")));
...
static void _neo_render_mode_03(const struct render_target_t *target) {
  ...
  uint8_t *p_data = (uint8_t *)target->dst;
  uint32_t index = 0;
  for (int y=0; y<100; y++)
  {
    index = y * 480 * 3;
    for (int x=0; x<100; x++)
    {
      p_data[index + x*3 + 0] = 0x00; // B
      p_data[index + x*3 + 1] = 0x00; // G
      p_data[index + x*3 + 2] = 0xFF;
    }
  }
}
#else
...
static void _neo_render_mode_03(const struct render_target_t *target) {
  ...
  for (int y=0; y<272; y++)
  {
    for (int x=0; x<480; x++)
    {
      color = target->dst[y*pitch + x];
      _frame_buf[y*480*3 + x*3 + 2] = color >> 16;
      _frame_buf[y*480*3 + x*3 + 1] = color >> 8;
      _frame_buf[y*480*3 + x*3 + 0] = color >> 0;
    }
  }
}
