...
static bool _read(const uint8_t num, uint8_t *dst, const uint32_t count);
static bool _write(const uint8_t num, const uint8_t *src, const uint32_t count);
static bool _open(const uint8_t num, const char *path);
