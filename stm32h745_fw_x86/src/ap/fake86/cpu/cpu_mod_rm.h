...
#define GET_CODE16(TYPE, OFFSET)   ((TYPE)((code[OFFSET+1]<<8) | (code[OFFSET]<<0)))
...
//  seg = _get_seg(CPU_SEG_SS) << 4;
  ...
  case 0:
    if (m->rm == 6) {
      m->num_bytes = 3;
      m->ea = seg + GET_CODE16(uint16_t, 2);
    }
    else {
      m->num_bytes = 1;
      m->ea = seg + addr;
  ...
  case 2:
    m->num_bytes = 3;
    m->ea = seg + addr + GET_CODE16(uint16_t, 2);
    break;
  default:
    UNREACHABLE();
  }
