...
OPCODE(_05) {
  ...
  const uint16_t rhs = GET_CODE16(uint16_t, 1);
  const uint16_t tmp = lhs + rhs;
  ADD_FLAGS_W(lhs, rhs, tmp);
  cpu_regs.ax = tmp;
  _step_ip(3);
...
OPCODE(_1D) {
  const uint16_t lhs = cpu_regs.ax;
  const uint16_t rhs = GET_CODE16(uint16_t, 1) + cpu_flags.cf;
...
// MOV AL, [imm16]
OPCODE(_A0) {
  const uint16_t imm = GET_CODE16(uint16_t, 1);
...
OPCODE(_C2) {
  const uint16_t disp16 = GET_CODE16(uint16_t, 1);
  cpu_regs.ip = _pop_w();
  cpu_regs.sp += disp16;
}
...
// MOV ax, imm16
OPCODE(_B8) {
  cpu_regs.ax = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV cx, imm16
OPCODE(_B9) {
  cpu_regs.cx = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV dx, imm16
OPCODE(_BA) {
  cpu_regs.dx = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV bx, imm16
OPCODE(_BB) {
  cpu_regs.bx = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV sp, imm16
OPCODE(_BC) {
  cpu_regs.sp = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV bp, imm16
OPCODE(_BD) {
  cpu_regs.bp = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV si, imm16
OPCODE(_BE) {
  cpu_regs.si = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}

// MOV di, imm16
OPCODE(_BF) {
  cpu_regs.di = GET_CODE16(uint16_t, 1);
  _step_ip(3);
}
...
OPCODE(_E8) {
  ...
  _push_w(cpu_regs.ip);
  // set new ip
  cpu_regs.ip += GET_CODE16(uint16_t, 1);
}

// JMP disp16 - jump with signed word displacement
OPCODE(_E9) {
  cpu_regs.ip += 3 + GET_CODE16(uint16_t, 1);
}

// JMP far - intersegment jump
OPCODE(_EA) {
  cpu_regs.ip = GET_CODE16(uint16_t, 1);
  cpu_regs.cs = GET_CODE16(uint16_t, 3);
}
