...
uint8_t RAM[0x100000] __attribute__((section(".sdram_buf")));
...
void writew86(uint32_t addr32, uint16_t value) {
  ...
  if (addr32 < 0xA0000) {
    ...
    RAM[addr32 + 0] = value;
    RAM[addr32 + 1] = value>>8;
    //memcpy(RAM + addr32, (void *)&value, 2);
  }
  else {
...
uint32_t mem_loadrom_flash(uint32_t addr32, uint8_t *p_data, uint32_t length)
{
  log_printf(LOG_CHAN_MEM, "loading rom at %08x, %dKB", addr32, length/1024);

  memcpy(&RAM[addr32], p_data, length);
  return length;
}
...
uint32_t mem_loadbios_flash(uint8_t *p_data, uint32_t length)
{
  uint32_t readsize = length;

  const uint32_t addr32 = 0x100000 - readsize;
  // log message
  log_printf(LOG_CHAN_MEM, "loading bios at %08x, %d KB", addr32, length/1024);
