...
void dxlcmdCmdif(void)
{
  ...
  if (cmdifGetParamCnt() == 1)
  {
    ...
    else if (cmdifHasString("close", 0) == true)
    {
      if (dxlIsOpen(&dxl_cmd) == true)
      {
    ...
    else if(cmdifHasString("ping", 0) == true)
    {
    ...
    else if(cmdifHasString("monitor", 0) == true)
    {
      uint8_t data;
  ...
  {
    number = (uint32_t)cmdifGetParam(1);


    if(cmdifHasString("ping", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
    ...
    else if(cmdifHasString("reboot", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
  ...
  else if (cmdifGetParamCnt() == 3)
  {
    ...
    port = (uint32_t) cmdifGetParam(1);
    baud = (uint32_t) cmdifGetParam(2);
    ...
    if (cmdifHasString("open", 0) == true)
    {
      dxlInit(&dxl_cmd, DXL_PACKET_VER_2_0);
      ret = dxlOpenPort(&dxl_cmd, port, baud);
      if (ret == true)
      {
        cmdifPrintf("open is OK Ver2.0, _DEF_DXL%d, %d bps", port+1, baud);
      }
      else
      {
        cmdifPrintf("open is FAIL, _DEF_DXL%d, %d bps", port+1, baud);
      }
    }
    if (cmdifHasString("open1", 0) == true)
    {
      dxlInit(&dxl_cmd, DXL_PACKET_VER_1_0);
      ret = dxlOpenPort(&dxl_cmd, port, baud);
      if (ret == true)
      {
        cmdifPrintf("open is OK Ver1.0, _DEF_DXL%d, %d bps", port+1, baud);
      }
      else
      {
        cmdifPrintf("open is FAIL, _DEF_DXL%d, %d bps", port+1, baud);
  ...
  else if (cmdifGetParamCnt() == 4)
  {
    number = (uint32_t) cmdifGetParam(1);
    addr   = (uint16_t) cmdifGetParam(2);
    length = (uint16_t) cmdifGetParam(3);

    if (cmdifHasString("read", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
  ...
  else if (cmdifGetParamCnt() == 5)
  {
    ...
    s_id   = (uint8_t) cmdifGetParam(1);
    e_id   = (uint8_t) cmdifGetParam(2);
    addr   = (uint16_t) cmdifGetParam(3);
    length = (uint16_t) cmdifGetParam(4);

    if (cmdifHasString("write", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
      uint32_t tx_data;

      s_id    = (uint8_t) cmdifGetParam(1);
      addr    = (uint16_t) cmdifGetParam(2);
      tx_data = (uint32_t) cmdifGetParam(3);
      length  = (uint16_t) cmdifGetParam(4);
    ...
    else if (cmdifHasString("syncread", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
    ...
    else if (cmdifHasString("bulkread", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
  ...
  else if (cmdifGetParamCnt() == 6)
  {
    ...
    e_id   = (uint8_t) cmdifGetParam(2);
    addr   = (uint16_t) cmdifGetParam(3);
    data   = (uint32_t) cmdifGetParam(4);
    length = (uint16_t) cmdifGetParam(5);

    if (cmdifHasString("syncwrite", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
    ...
    else if (cmdifHasString("bulkwrite", 0) == true && dxlIsOpen(&dxl_cmd) == true)
    {
      uint8_t *p_data = (uint8_t *)&data;
  ...
  if (ret == false)
  {
    ...
    cmdifPrintf( "dxlcmd open port(1~n) baud\n");
